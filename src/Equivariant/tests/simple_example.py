...
def spherical_harmonics(lmax: int, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
    """
    Express a vector in terms of spherical harmonics to a certain order
    :param lmax: the maximum order
    :param x:
    :param y:
    :param z:
    :return:
...
if __name__ == '__main__':
    ...
    lmax = 1
    x = spherical_harmonics(lmax,v[0],v[1],v[2]) #express v in terms of spherical harmonics up to a certain order
    y = spherical_harmonics(lmax,u[0],u[1],u[2]) #express v in terms of spherical harmonics up to a certain order

    #Now lets do the tensorproduct of these two x is a direct sum of Y00 and Y10 and similar for y.

    #We get different components from this:

    #We get l=0 components:
    z[0] = x[0]*y[0]
    z[0] += - math.sqrt(1/3) * (x[1]*y[1] + x[2]*y[2] + x[3]*y[3])
    ...
    rot = o3.rand_matrix(1) #Gives a random rotation matrix
