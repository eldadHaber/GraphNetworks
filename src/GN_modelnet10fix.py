...
def transferWeights(smallmodel, largemodel, interp=True):
    ...
    if interp:
        ...
        K1 = torch.nn.functional.interpolate(K1, size=[2 * K1.shape[2], K1.shape[3], K1.shape[4]], mode='trilinear',
                                             align_corners=True).squeeze()
        K2 = torch.nn.functional.interpolate(K2, size=[2 * K2.shape[2], K2.shape[3], K2.shape[4]], mode='trilinear',
                                             align_corners=True).squeeze()
...
for epoch in range(1, 201):
    ...
    if (nlayer < 8) and (epoch % 10 == 9):
        nlayer = nlayer * 2
        # h = h / 2
        model_new = GN.graphNetwork_nodesOnly(nNin, nopen, nhid, nNclose, nlayer, h=h, dense=False, varlet=True,
                                              wave=wave,
