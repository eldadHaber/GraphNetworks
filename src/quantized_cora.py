...
if not realVarlet:
    optimizer = torch.optim.Adam([
        ...
        dict(params=model.final_activation_alpha, lr=5e-4, weight_decay=0),
        dict(params=model.final_activation_alpha_grad, lr=5e-4, weight_decay=0)
...
def train():
    ...
    with torch.no_grad():
        [outNQ, GNQ, actvals_NOquant] = model(xn, G, runWOQuant=True)
    ...
    print("TRAIN MSE:", F.mse_loss(actvals_quant, actvals_NOquant).item(), "NORM:",
          torch.norm(actvals_quant - actvals_NOquant).item())
    print("TRAIN MSE PER LAYER:", (actvals_quant - actvals_NOquant).norm(dim=[1, 2]))
    beta = 0
    betas.append(beta)
...
def test():
    ...
    actvals_quant = torch.cat(actvals_quant)
    actvals_NOquant = torch.cat(actvals_NOquant)
    print("TEST MSE:", F.mse_loss(actvals_quant, actvals_NOquant).item(), "NORM:",
          torch.norm(actvals_quant - actvals_NOquant).item())
    print("TEST MSE PER LAYER:", (actvals_quant - actvals_NOquant).norm(dim=[1, 2]))
